...
// 사용자 목록을 표시할 영역을 team_create.html에서 가져오기
// member-list라는 빈 컨테이너를 가져옴
const memberList = document.getElementById("member-list");

// 팀원 추가 버튼 클릭
addMemberButton.addEventListener("click", async () => {

    // Flask에 해당 주차에서 선택 가능한 사용자 목록 요청
    ...
    data.users.forEach((user) => {
        ...
        // 이 체크박스가 어떤 사용자인지 user_id 연결
        checkbox.value = user.user_id;

        // 현재 로그인한 사용자인 경우 자동 선택
...
createTeamButton.addEventListener("click", async () => {
    ...
    // Flask의 /api/team_pages로 POST 요청
...
// 취소 버튼 요소를 team_create.html에서 가져오기
